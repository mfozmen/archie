...
#
# Scans (1) all tracked files, (2) all commit messages not yet on any remote.
# Exits 1 on any hit. If no pattern file exists, warns and exits 0.
set -euo pipefail
...
while IFS= read -r pat; do
  # 1) tracked files
  if hits="$(git grep -I -i -n -E -- "$pat" 2>/dev/null)"; then
