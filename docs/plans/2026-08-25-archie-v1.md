...
- Node ≥ 18; **zero npm dependencies** (dev and runtime). Tests use built-in `node --test`, invoked with NO path argument so discovery never depends on the shell. CI runs a matrix over Node 18, 22 and 24 — verify version-sensitive behavior against the floor, not just your local Node.
...
**Interfaces:**
- Produces: `npm test` → `node --test` (NO path argument — Node's built-in default discovery finds `tests/*.test.js` on both Node 18 and 26; a bare directory arg is treated as a module path and fails, and a `tests/*.test.js` glob would depend on the invoking shell); `helpers.makeTempRepo()` → `{root}` a real git repo in a temp dir with one initial commit; `helpers.write(root, relPath, content)`; `helpers.commitAll(root, msg)`.
...
- [ ] **Step 2: Run to verify it fails** — `node --test` → FAIL (`Cannot find module './helpers'`).
...
Add this to `tests/staleness.test.js` — it pins the honesty property, not just the plumbing:
...
const { run, tryRun } = require('./lib/exec');
...
test('rg and grep paths agree on the same fixture', () => {
  ...
  execFileSync('git', ['-C', root, '-c', 'user.email=t@example.com', '-c', 'user.name=T',
    'commit', '-qm', 'fixture']);
...
const fs = require('node:fs');
const path = require('node:path');
const { run, tryRun, hasBin } = require('./lib/exec');   // `run` is used by trackedFiles
...
// ponytail: kept only as the untracked-repo escape hatch; trackedFiles is the norm.
...
// Same file list as grepProbe (Ruling 2) — never let rg walk the tree itself, or the
// result depends on whether ripgrep is installed. --no-ignore --hidden disable rg's
// own filtering; `git ls-files` already decided what counts.
...
function sweep(root, recipe, opts = {}) {
  ...
  const files = trackedFiles(root);   // ONE list for both paths — see Ruling 2
...
**ponytail: the tracked-file list is spread into rg's argv, which has an OS ceiling (`E2BIG`).** On a repo with tens of thousands of tracked files this RAISES rather than silently truncating — correct per the honesty rule, but a real ceiling. Upgrade path when it bites: chunk the file list and merge the hits. Do NOT "fix" it by letting rg walk the tree again — that reintroduces the install-dependent divergence Ruling 2 closed.
...
- [ ] **Step 1:** CI already exists on this branch (`.github/workflows/test.yml`: checkout@v5, a Node `[18, 22, 24]` matrix, `npm test` + `bash scripts/leak-scan.test.sh`, master-only push trigger, PR-scoped concurrency cancel). **Do NOT recreate or overwrite it** — a plan-driven rewrite would silently drop the matrix that the Global Constraints require. Verify it matches that description and move on. Note that branch protection requires the matrix check names `test (18)`, `test (22)`, `test (24)`; changing the matrix means updating protection in lockstep.
