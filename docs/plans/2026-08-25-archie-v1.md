...
- Common preamble in every SKILL.md: resolve repo root (`git rev-parse --show-toplevel`); if `.archie/config.json` missing → run first-run setup (spec §5): output language, scope via `scripts/scope.js` candidates + confirmation, and the wiki output directory; store it with `scripts/store.js`; narrative in configured language, identifiers never translated.
