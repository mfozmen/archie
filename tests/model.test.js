...
test('outputDir defaults to the wiki directory under .archie', () => {
  const { root } = makeTempRepo();
  assert.strictEqual(M.outputDir(M.storeFor(root), root, M.storeFor(root)), path.join(root, '.archie', 'wiki'));
  M.saveConfig(M.storeFor(root), { output: 'docs/system-map' });
  assert.strictEqual(M.outputDir(M.storeFor(root), root, M.storeFor(root)), path.join(root, 'docs', 'system-map'));
});
...
test('output must not resolve to the repository root', () => {
  ...
  assert.strictEqual(M.outputDir(M.storeFor(root), root, M.storeFor(root)), path.join(root, 'docs', 'map'));
});
